...
@pytest.fixture
def s5_loop_business_reference_number_filter() -> S5LoopBusinessInstructionAndReferenceNumbersTransactionFilter:
...
def test_s5_loop_n1_loop_filter(
    ...
    transaction_sets: dict[str, Any] = get_transaction_transaction_sets(transaction_data=actual_result[0])

    assert len(actual_result) == 3
    assert "QN" == (
        transaction_sets
        ["S5_loop"][0]
        ["businessInstructionsAndReferenceNumber"][0]
